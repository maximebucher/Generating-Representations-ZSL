...
class adv_ae(object):
    def __init__(self, args, encoder_size, decoder_size, discriminator_size, data_config):
        self.count_data = 0
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('Dis') as scope:
            Dis_fake = self.discriminator(pred_noise)
            scope.reuse_variables()
            Dis_true = self.discriminator(self.z_pl)

        self.loss_e = tf.reduce_mean(tf.nn.l2_loss(self.x_pl - pred_x))

        dis_loss_true = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(Dis_true), logits=Dis_true))
        dis_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(Dis_fake), logits=Dis_fake))
        self.loss_dis = dis_loss_true + dis_loss_fake

        self.loss_g = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(Dis_fake), logits=Dis_fake))

        vars = tf.trainable_variables()
        e_params = [v for v in vars if v.name.startswith('E/') or v.name.startswith('D/')]
        dis_params = [v for v in vars if v.name.startswith('Dis/')]
        g_params = [v for v in vars if v.name.startswith('E/')]

        self.opt_e = self.optimizer(self.loss_e, e_params)
        self.opt_dis = self.optimizer(self.loss_dis, dis_params)
    ...
    def encoder(self, input):
        ...
        for i, size in enumerate(self.encoder_size):
            ...
            input = tf.nn.dropout(self.lrelu(input_lin), 1.0-self.drop_out_rate)
    ...
    def discriminator(self, input):
        for i, size in enumerate(self.discriminator_size):
            input_lin, w, b = self.linear(input, size, name='dis' + str(i))
            input = tf.nn.dropout(self.lrelu(input_lin), 1.0 - self.drop_out_rate_pl)
        h, w, b = self.linear(input, 1, name='dis' + str(len(self.discriminator_size)))
        return h
    ...
    def next_batch(self, start, end):
        ...
        return self.features[start:end], self.attributes[start:end], self.labels[start:end], np.random.normal(0.0, 1.0, (end-start, self.noise_size))
    ...
    def train(self):
        with tf.Session() as self.session:
            ...
            for epoch in xrange(self.num_epoch):
                mean_loss_e = mean_loss_dis = mean_loss_g = 0.0
                for count in xrange(0, self.features.shape[0], self.batch_size):
                    features_batch, attributes_batch, labels_batch, noise_batch = self.next_batch(count, count+self.batch_size)
                    # update auto-encoder
                    ...
                    loss_dis, _ = self.session.run([self.loss_dis, self.opt_dis], {
                        self.x_pl: features_batch,
                    ...
                    mean_loss_e += loss_e
                    mean_loss_dis += loss_dis
                    mean_loss_g += loss_g

                mean_loss_e /= (self.features.shape[0] / self.batch_size)
                mean_loss_dis /= (self.features.shape[0] / self.batch_size)
                mean_loss_g /= (self.features.shape[0] / self.batch_size)

                print 'epoch : {}: D : {}: Dis : {}: G : {}'.format(epoch, mean_loss_e, mean_loss_dis, mean_loss_g)
    ...
    def val(self):
        ...
        if acc > self.best_acc:
            ...
            self.best_acc = acc
            self.last_file_name = "model_weights_adv_gan/" + self.name + "_" \
                                  + str(self.learning_rate) + "_" \
                                  + str(self.encoder_size) + "_" \
                                  + str(self.decoder_size) + "_" \
                                  + str(self.discriminator_size) + "_" \
...
def main():
    ...
    model = adv_ae(args, encoder_size, decoder_size, discriminator_size, data_config)
    model.train()
