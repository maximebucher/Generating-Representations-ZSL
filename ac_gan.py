...
class gan(object):
    def __init__(self, args, gen_size, dis_size, data_config):
        self.count_data = 0
        ...
        if self.validation:
            ...
            if self.generalized:
                features_test_seen = np.load(data_config['dir_path'] + "" + data_config['img_test_seen_file'])
        ...
        # generator input => attributes + noise
        ...
        self.labels_pl = tf.placeholder(tf.float32, shape=(None, self.labels.shape[1]))
        self.batch_size_pl = tf.placeholder(tf.int32)
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('G'):
            self.G = self.generator(self.attributes_pl, self.z_pl)

        # Discriminator
        with tf.variable_scope('D') as scope:
            self.D1, pred_real = self.discriminator(self.x_pl)
            scope.reuse_variables()
            self.D2, pred_fake = self.discriminator(self.G)

        self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.fill([self.batch_size_pl, 1], 1.0), logits=self.D1)) \
                           + tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_pl, logits=pred_real))

        self.d_loss_fake = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(self.D2), logits=self.D2)) \
                           + tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_pl, logits=pred_fake))
        self.loss_d = self.d_loss_real + self.d_loss_fake

        self.loss_g = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.D2), logits=self.D2)) \
                     + tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_pl, logits=pred_fake))
        ...
        self.opt_d = self.optimizer(self.loss_d, self.d_params)
        self.opt_g = self.optimizer(self.loss_g, self.g_params)

    def discriminator(self, input):
        input = tf.nn.dropout(input, 1.0 - self.drop_out_rate_input)
        for i, size in enumerate(self.dis_size):
            input_lin, w, b = self.linear(input, size, name='dis' + str(i))
            input = tf.nn.dropout(self.lrelu(input_lin), 1.0 - self.drop_out_rate_pl)
        h, w, b = self.linear(input, 1, name='dis' + str(len(self.dis_size)))
        pred, wp, bp = self.linear(input, self.labels.shape[1], name='pred')
        return h, pred

    def generator(self, attributes, code):
        input = tf.concat([attributes, code], 1)
        for i, size in enumerate(self.gen_size):
            input_lin, w, b = self.linear(input, size, name='g' + str(i))
            input = tf.nn.dropout(self.lrelu(input_lin), 1.0 - self.drop_out_rate_pl)
        h, w, b = self.linear(input, self.features.shape[1], name='g' + str(len(self.gen_size)))
        return h

    def linear(self, input, output_dim, name=None, stddev=0.01):
        with tf.variable_scope(name or 'linear'):
            norm = tf.random_normal_initializer(stddev=stddev)
            const = tf.constant_initializer(0.01)
            w = tf.get_variable('w', [input.get_shape()[1], output_dim], initializer=norm)
            b = tf.get_variable('b', [output_dim], initializer=const)
    ...
    def optimizer(self, loss, var_list):
        ...
        optimizer = tf.train.AdamOptimizer(self.lr_pl).minimize(
            loss,
            global_step=batch,
            var_list=var_list
        )
    ...
    def next_batch(self, start, end):
        ...
        noise = np.random.normal(0.0, 1.0, (end-start, self.noise_size))
        return self.features[start:end], self.labels[start:end], self.attributes[start:end], noise
        ...
        with tf.Session() as self.session:
            ...
            for epoch in xrange(self.num_epoch):
                ...
                for count in xrange(0, self.features.shape[0], self.batch_size):
                    features_batch, labels_batch, attributes_batch, noise_batch = self.next_batch(count, count+self.batch_size)
                    ...
                    # update generator
                    loss_g, _ = self.session.run([self.loss_g, self.opt_g], {
                        self.x_pl: features_batch,
                        self.z_pl: noise_batch,
                        self.labels_pl: labels_batch,
                        self.attributes_pl: attributes_batch,
                ...
                mean_loss_g /= (self.features.shape[0] / self.batch_size)

                print 'epoch : {}: D : {}: G : {}'.format(epoch, mean_loss_d, mean_loss_g)
    ...
    def generate_fake_image_val(self, attributes_class, labels_class, nb_ex):
        ...
        for c in xrange(labels_class.shape[0]):
            ...
            features[c * nb_ex:(c * nb_ex) + nb_ex] = self.session.run(self.G, {
                self.z_pl: noise,
    ...
    def val(self):
        ...
        if acc > self.best_acc:
            ...
            self.best_acc = acc
            self.last_file_name = "model_weights_gan/" + self.name + "_" \
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run gan experiments on various ZSL datasets")
    ...
    model = gan(args, gen_size, dis_size, data_config)
    model.train()
